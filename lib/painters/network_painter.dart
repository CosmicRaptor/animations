...
class ParticleNetwork extends StatefulWidget {
  ...
  const ParticleNetwork({super.key, required this.numParticles, required this.speed});
...
class _ParticleNetworkState extends State<ParticleNetwork> with SingleTickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    particles = List.generate(
      widget.numParticles,
        ...
        velocity: Offset((rand.nextDouble() - 0.5) * widget.speed, (rand.nextDouble() - 0.5) * widget.speed),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    if (particles == null) return const SizedBox(); // prevent null error on first build
    return CustomPaint(
      size: Size.infinite,
